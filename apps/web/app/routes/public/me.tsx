...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  const [draftRecords, autosaveDrafts, mySentences, myQuestions, unansweredQuestions, myRecords] = await database.batch([
    database
...
export default function MySpacePage({ loaderData }: Route.ComponentProps) {
  const { learner, draftRecords, autosaveDrafts, mySentences, myQuestions, unansweredQuestions, myResponses, mySavedRecords, myRecords } = loaderData;
          ...
          <div className="space-y-16">
            <section>
              ...
              {myResponses.length === 0 ? (
                ...
                <div className="flex flex-col gap-5">
                  {myResponses.map((response: typeof myResponses[number]) => (
                    <div key={response.id} className="relative group">
                      <ResponseCard
                        response={{
                          ...
                          moderationStatus: response.moderationStatus ?? undefined,
                        }}
