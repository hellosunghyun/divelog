...
INSERT INTO collaboration_members (unit_id, learner_id, role, joined_at) VALUES
  ('collab-unit-alpha', 'usr-seed-minjun', 'lead', unixepoch() - 86400 * 12),
  ('collab-unit-alpha', 'usr-seed-soyeon', 'member', unixepoch() - 86400 * 11),
  ('collab-unit-alpha', 'usr-seed-hyunjin', 'member', unixepoch() - 86400 * 11);
...
INSERT INTO records (
  ...
  ('record-020', 'hana-draft', 'usr-seed-hana', 'stage-challenge-1', NULL, NULL, NULL, '[초안] 아직 정리되지 않은 생각', '문장이 아직 매끄럽지 않지만 지금의 흐릿함도 남겨 두고 싶었습니다. 나중에 다시 읽으면 어디에서 머뭇거렸는지 더 잘 보일 것 같습니다.', '문장이 아직 매끄럽지 않지만 지금의 흐릿함도 남겨 두고 싶었습니다. 나중에 다시 읽으면 어디에서 머뭇거렸는지 더 잘 보일 것 같습니다.', 'note', 'personal', 'free', 'draft', 'closed', 0, 'clean', 'cohort-2026', unixepoch() - 3600 * 12, unixepoch() - 3600 * 12),
  ('record-021', 'soyeon-short', 'usr-seed-soyeon', 'stage-challenge-1', NULL, NULL, NULL, '오늘의 한 줄', '탐구는 정답을 서둘러 고르는 일이 아니라 더 오래 남는 질문을 찾는 일에 가깝다.', '탐구는 정답을 서둘러 고르는 일이 아니라 더 오래 남는 질문을 찾는 일에 가깝다.', 'note', 'personal', 'sprint', 'cohort', 'open', 0, 'clean', 'cohort-2026', unixepoch() - 86400 * 2, unixepoch() - 86400 * 2),
...
INSERT INTO tags (id, name, slug, description, color, created_by, created_at, updated_at) VALUES
  ('tag-001', '기술', 'tech', '기술 관련 기록과 탐구', '#146C94', 'usr-seed-hana', unixepoch() - 86400 * 30, unixepoch() - 86400 * 30),
  ('tag-002', '회고', 'retrospect', '반성과 성찰의 기록', '#7C3AED', 'usr-seed-hana', unixepoch() - 86400 * 30, unixepoch() - 86400 * 30),
  ('tag-003', '협업', 'collaboration', '함께 탐구하는 경험과 배움', '#EC4899', 'usr-seed-minjun', unixepoch() - 86400 * 28, unixepoch() - 86400 * 28),
  ('tag-004', '질문', 'question', '오래 붙드는 질문과 탐구', '#F59E0B', 'usr-seed-jiwon', unixepoch() - 86400 * 25, unixepoch() - 86400 * 25),
  ('tag-005', '기록의 의미', 'meaning-of-writing', '기록을 남기는 이유와 역할', '#10B981', 'usr-seed-soyeon', unixepoch() - 86400 * 22, unixepoch() - 86400 * 22),
  ('tag-006', '전환', 'transition', '구간의 전환과 변화', '#8B5CF6', 'usr-seed-hyunjin', unixepoch() - 86400 * 20, unixepoch() - 86400 * 20),
  ('tag-007', '학습', 'learning', '학습의 과정과 의미', '#06B6D4', 'usr-seed-jaemin', unixepoch() - 86400 * 18, unixepoch() - 86400 * 18);
...
INSERT INTO self_answers (id, question_id, author_id, content, created_at, updated_at) VALUES
  ('sa-001', 'q-001', 'usr-seed-hana', '처음 떠오른 질문은 내가 무엇을 잘하고 싶은지가 아니라 무엇을 오래 붙들 수 있는지에 관한 것이었습니다. 그 질문을 인정하자 탐색이 조금 덜 막막해졌습니다.', unixepoch() - 86400 * 7, unixepoch() - 86400 * 7),
  ('sa-002', 'q-002', 'usr-seed-hana', '완성되지 않은 상태를 견디게 한 것은 작은 기록이었습니다. 짧은 메모라도 남겨 두면 다음 날 다시 이어갈 수 있었습니다.', unixepoch() - 86400 * 6, unixepoch() - 86400 * 6),
  ('sa-003', 'q-005', 'usr-seed-hana', '전환점에서 가장 크게 달라진 것은 모른다는 상태를 두려움만으로 보지 않게 되었다는 점입니다. 이제는 그 상태를 다음 질문의 시작으로 읽을 수 있습니다.', unixepoch() - 86400 * 28, unixepoch() - 86400 * 28);
...
INSERT INTO responses (
  ...
  ('resp-001', 'record-001', 'q-001', 'usr-seed-jiwon', 'resonance', '막막함을 시작점으로 읽어낸 문장이 많이 와닿았습니다. 저도 처음에는 모른다는 사실이 부끄러웠는데, 지금은 그 모름이 질문의 자리라고 느끼고 있습니다.', 'cohort', 'clean', unixepoch() - 86400 * 7, unixepoch() - 86400 * 7),
  ('resp-002', 'record-001', 'q-001', 'usr-seed-minjun', 'question', '막연한 답답함을 따라가 보기로 결심하게 만든 장면이 있었나요?', 'cohort', 'clean', unixepoch() - 86400 * 6, unixepoch() - 86400 * 6),
  ('resp-003', 'record-002', 'q-003', 'usr-seed-soyeon', 'connection', '저도 혼자 탐구할 때 기록이 가장 든든한 동반자였습니다. 쓰는 행위가 외로움을 없애지는 않지만, 외로움을 설명할 언어를 만들어 주었습니다.', 'cohort', 'clean', unixepoch() - 86400 * 5, unixepoch() - 86400 * 5),
  ('resp-004', 'record-006', 'q-004', 'usr-seed-hyunjin', 'resonance', '학습이 무엇을 바꾸는가라는 질문이 오래 남습니다. 저는 요즘 학습이 나를 대하는 말투를 바꾸는 일일 수도 있다고 생각합니다.', 'cohort', 'clean', unixepoch() - 86400 * 2, unixepoch() - 86400 * 2),
  ('resp-005', 'record-012', 'q-006', 'usr-seed-jaemin', 'resonance', '기록이 거울처럼 느껴진다는 표현에 많이 공감했습니다. 저는 지난 메모를 다시 읽을 때 그날의 망설임이 더 선명하게 보이곤 했습니다.', 'cohort', 'clean', unixepoch() - 86400 * 12, unixepoch() - 86400 * 12),
  ('resp-006', 'record-014', 'q-007', 'usr-seed-jiwon', 'suggestion', '질문을 오래 들고 가기 어렵다면 같은 질문을 다른 날의 문장으로 다시 적어 보는 것도 좋았습니다. 질문의 결이 조금씩 바뀌는 것이 보였습니다.', 'cohort', 'clean', unixepoch() - 86400 * 10, unixepoch() - 86400 * 10),
  ('resp-007', 'record-017', 'q-008', 'usr-seed-minjun', 'question', '미래를 준비하는 기록이라면, 지금의 나는 무엇을 잊지 않기 위해 쓰고 있는 걸까요?', 'cohort', 'clean', unixepoch() - 86400 * 18, unixepoch() - 86400 * 18),
  ('resp-008', 'record-022', 'q-009', 'usr-seed-soyeon', 'connection', '저도 팀 안에서 다름이 걸림돌이 아니라 자원이 되는 순간을 경험했습니다. 서로 다른 문장이 한 질문을 더 넓게 만들어 주었습니다.', 'cohort', 'clean', unixepoch() - 86400 * 14, unixepoch() - 86400 * 14),
  ('resp-009', 'record-023', 'q-010', 'usr-seed-hyunjin', 'resonance', '실패를 다시 읽는 일의 필요성을 저도 자주 느낍니다. 지나간 일을 지우지 않고 다시 보는 태도가 다음 시도를 다르게 만든다고 생각합니다.', 'cohort', 'clean', unixepoch() - 86400 * 8, unixepoch() - 86400 * 8),
  ('resp-010', 'record-001', NULL, 'usr-seed-soyeon', 'resonance', '처음의 흐릿한 마음을 솔직하게 남겨 주셔서 좋았습니다. 정리되지 않은 상태가 오히려 더 진하게 전달되었습니다.', 'cohort', 'clean', unixepoch() - 86400 * 5, unixepoch() - 86400 * 5),
  ('resp-011', 'record-008', 'q-005', 'usr-seed-jiwon', 'connection', '전환을 서서히 바뀌는 시선이라고 표현한 부분이 인상 깊었습니다. 저도 급격한 변화보다 문장을 읽는 방식이 달라졌을 때 전환을 느꼈습니다.', 'cohort', 'clean', unixepoch() - 86400 * 27, unixepoch() - 86400 * 27),
  ('resp-012', 'record-025', 'q-011', 'usr-seed-minjun', 'question', '질문의 변화가 가장 크게 느껴졌던 순간은 언제였나요?', 'cohort', 'clean', unixepoch() - 86400 * 58, unixepoch() - 86400 * 58),
  ('resp-013', 'record-003', 'q-012', 'usr-seed-jaemin', 'suggestion', '저는 진짜 궁금한 것을 찾기 위해 먼저 마음이 움직이지 않는 질문들을 적어 보고 지웠습니다. 남는 질문이 의외로 또렷했습니다.', 'cohort', 'clean', unixepoch() - 86400 * 3, unixepoch() - 86400 * 3),
  ('resp-014', 'record-005', NULL, 'usr-seed-jiwon', 'resonance', '함께 읽을 때 보이는 장면이 달라진다는 말이 기억에 남습니다. 저도 누군가의 시선이 들어올 때 기록이 넓어진다고 느낍니다.', 'cohort', 'clean', unixepoch() - 86400 * 1, unixepoch() - 86400 * 1),
  ('resp-015', 'record-009', NULL, 'usr-seed-hyunjin', 'connection', '모르는 지점을 더 선명하게 본다는 문장이 제 경험과도 닮아 있습니다. 깊어지는 탐구는 답을 많이 아는 일이 아니라 질문을 더 정확히 보는 일 같았습니다.', 'cohort', 'clean', unixepoch() - 86400 * 4, unixepoch() - 86400 * 4);

INSERT INTO sentences (id, record_id, saved_by_id, content, reason, paragraph_index, created_at) VALUES
  ('sent-001', 'record-001', 'usr-seed-jiwon', '완성된 문장이 없어도 질문은 자랄 수 있다는 사실을 조금씩 알게 되었습니다.', '질문을 두려워하지 않게 만드는 문장이라 저장했습니다.', 0, unixepoch() - 86400 * 7),
  ('sent-002', 'record-002', 'usr-seed-soyeon', '기록이 나를 다시 불러 세운다는 사실이 든든했습니다.', '기록의 역할을 간결하게 보여 주는 문장입니다.', 0, unixepoch() - 86400 * 5),
  ('sent-003', 'record-006', 'usr-seed-hyunjin', '학습이 결국 무엇을 바꾸는가라는 질문이었습니다.', '구간 전체를 관통하는 질문처럼 느껴졌습니다.', 0, unixepoch() - 86400 * 2),
  ('sent-004', 'record-008', 'usr-seed-minjun', '전환은 갑작스러운 도약보다 서서히 바뀌는 시선에 더 가까웠습니다.', '전환의 감각을 차분하게 설명해 준 문장입니다.', 0, unixepoch() - 86400 * 28),
  ('sent-005', 'record-012', 'usr-seed-jaemin', '기록은 결과물이 아니라 다시 생각하게 만드는 장치라고 느낍니다.', '기록의 기능을 새롭게 이해하게 된 문장입니다.', 0, unixepoch() - 86400 * 13),
  ('sent-006', 'record-017', 'usr-seed-hana', '기록은 다음의 나에게 말을 거는 방식이기도 했습니다.', '기록을 남기는 이유를 오래 떠올리게 하는 문장입니다.', 0, unixepoch() - 86400 * 19),
  ('sent-007', 'record-022', 'usr-seed-soyeon', '각자의 다름이 걸림돌이 아니라 자원이라는 사실을 더 분명히 알게 되었습니다.', '협업의 태도를 잘 드러내는 문장이라 저장했습니다.', 0, unixepoch() - 86400 * 15),
  ('sent-008', 'record-025', 'usr-seed-jiwon', '질문의 변화 자체가 이 구간에서 내가 얻은 가장 큰 흔적입니다.', '성장을 비교가 아니라 질문의 변화로 말하는 방식이 좋았습니다.', 0, unixepoch() - 86400 * 59),
  ('sent-009', 'record-014', 'usr-seed-minjun', '질문은 곧바로 해결되지 않아도 나를 다시 움직이게 합니다.', '질문을 붙드는 태도를 잘 보여 줍니다.', 0, unixepoch() - 86400 * 10),
  ('sent-010', 'record-009', 'usr-seed-jaemin', '모르는 지점을 더 선명하게 보는 일이었습니다.', '탐구의 깊이를 설명하는 표현이 인상적이었습니다.', 0, unixepoch() - 86400 * 12),
  ('sent-011', 'record-021', 'usr-seed-hyunjin', '탐구는 정답을 서둘러 고르는 일이 아니라 더 오래 남는 질문을 찾는 일에 가깝다.', '한 문장으로 탐구의 태도를 또렷하게 보여 줍니다.', 0, unixepoch() - 86400 * 2),
  ('sent-012', 'record-018', 'usr-seed-hana', '이제는 그 상태를 다음 질문의 시작으로 읽을 수 있습니다.', '모름을 받아들이는 태도의 변화가 잘 담겨 있습니다.', 0, unixepoch() - 86400 * 35);
...
INSERT INTO notifications (
  ...
  ('notif-001', 'usr-seed-hana', 'response', '이하나님의 기록에 공명이 도착했습니다', '박지원님이 탐색의 첫 번째 노트에 공명을 남겼습니다.', 'record-001', NULL, 0, unixepoch() - 86400 * 7),
  ('notif-002', 'usr-seed-hana', 'response', '이하나님의 기록에 질문이 남겨졌습니다', '최민준님이 탐색의 첫 번째 노트에 질문을 남겼습니다.', 'record-001', 'q-001', 0, unixepoch() - 86400 * 6),
  ('notif-003', 'usr-seed-hana', 'response', '이하나님의 질문에 응답이 도착했습니다', '박지원님이 전환점에서 돌아보기의 질문에 연결을 남겼습니다.', 'record-008', 'q-005', 1, unixepoch() - 86400 * 27),
  ('notif-004', 'usr-seed-jiwon', 'response', '박지원님의 기록에 연결이 남겨졌습니다', '김현진님이 깊어지는 탐구에 연결점을 남겼습니다.', 'record-009', NULL, 0, unixepoch() - 86400 * 4),
  ('notif-005', 'usr-seed-soyeon', 'response', '정소연님의 기록에 공명이 도착했습니다', '김현진님이 지금 나에게 남겨진 질문에 공명을 남겼습니다.', 'record-006', 'q-004', 1, unixepoch() - 86400 * 2),
  ('notif-006', 'usr-seed-minjun', 'response', '최민준님의 기록에 연결이 남겨졌습니다', '정소연님이 팀과 함께 발견한 것들에 연결점을 남겼습니다.', 'record-005', NULL, 0, unixepoch() - 86400 * 1),
  ('notif-007', 'usr-seed-hana', 'memory', '첫 번째 전환 구간의 공동 기억이 발행되었습니다', '여러 기록과 문장이 하나의 공동 기억으로 엮였습니다.', NULL, NULL, 1, unixepoch() - 86400 * 19),
  ('notif-008', 'usr-seed-hyunjin', 'response', '김현진님의 질문에 응답이 도착했습니다', '오재민님이 나만의 탐구: 기록의 힘에 공명을 남겼습니다.', 'record-012', 'q-006', 0, unixepoch() - 86400 * 12);
...
INSERT INTO curation_slots (
  ...
  ('slot-005', 'learner', 'usr-seed-hana', 'learner', 0, 0, 0, unixepoch() - 86400 * 1, unixepoch() - 86400 * 1);
...
INSERT INTO audit_logs (
  ...
  ('audit-001', 'usr-seed-hana', 'record', 'record-001', 'created', NULL, '{"slug":"first-note"}', unixepoch() - 86400 * 9),
  ('audit-002', 'usr-seed-minjun', 'record', 'record-005', 'created', NULL, '{"slug":"minjun-collab"}', unixepoch() - 86400 * 4),
  ('audit-003', 'usr-seed-hana', 'record', 'record-008', 'created', NULL, '{"slug":"hana-reflection"}', unixepoch() - 86400 * 31),
  ('audit-004', 'usr-seed-hana', 'record', 'record-025', 'created', NULL, '{"slug":"hana-prelude-end"}', unixepoch() - 86400 * 61),
  ('audit-005', 'usr-seed-soyeon', 'record', 'record-017', 'created', NULL, '{"slug":"soyeon-writing"}', unixepoch() - 86400 * 21),
  ('audit-006', 'usr-seed-jiwon', 'record', 'record-009', 'created', NULL, '{"slug":"jiwon-deep"}', unixepoch() - 86400 * 13);
