...
function dateInputToTimestamp(value: string | null): number | null {
...
export async function action({ params, request, context }: Route.ActionArgs) {
  ...
  const startDateStr = f.get("startDate") as string | null;
  const endDateStr = f.get("endDate") as string | null;

  await db(context.cloudflare.env.DB).update(stages).set({
    name: f.get("name") as string,
    description: (f.get("description") as string) || null,
    status: f.get("status") as string,
    isCurrent: f.get("isCurrent") === "on",
    startDate: dateInputToTimestamp(startDateStr || null),
    endDate: dateInputToTimestamp(endDateStr || null),
...
export default function AdminStageEditPage({ loaderData }: Route.ComponentProps) {
  ...
  const labelClass = "block text-xs text-admin-text-secondary mb-1.5";
  const inputClass = "w-full px-3 py-2 rounded-sm border border-admin-border bg-admin-surface text-sm focus:ring-2 focus:ring-admin-accent outline-none";
  ...
  return (
    <div>
      ...
      <form method="post" className="flex flex-col gap-4 max-w-xl bg-admin-surface rounded-md p-6 border border-admin-border">
        <div>
          <label htmlFor="stage-name" className={labelClass}>이름</label>
          <input id="stage-name" name="name" defaultValue={stage.name} required className={inputClass} />
        </div>
        <div>
          <label htmlFor="stage-description" className={labelClass}>설명</label>
          <textarea id="stage-description" name="description" defaultValue={stage.description ?? ""} rows={3} className={`${inputClass} resize-y`} />
        </div>
        <div>
          <label htmlFor="stage-status" className={labelClass}>상태</label>
          <select id="stage-status" name="status" defaultValue={stage.status} className={inputClass}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="stage-start-date" className={labelClass}>시작일</label>
            <input id="stage-start-date" type="date" name="startDate" defaultValue={timestampToDateInput(stage.startDate)} className={inputClass} />
          </div>
          <div>
            <label htmlFor="stage-end-date" className={labelClass}>종료일</label>
            <input id="stage-end-date" type="date" name="endDate" defaultValue={timestampToDateInput(stage.endDate)} className={inputClass} />
          </div>
        </div>
        <div>
