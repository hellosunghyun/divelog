    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-8">
        {[
          ...
          <div key={panel.label} className={`bg-admin-surface rounded-lg border p-5 ${panel.urgent ? "border-error" : "border-admin-border"}`}>
            <p className="text-caption text-admin-text-secondary mb-2">{panel.label}</p>
            <p className={`text-2xl font-semibold ${panel.urgent ? "text-error" : "text-admin-text"}`}>{panel.value}</p>
          </div>
        ))}
      </div>
      {flaggedRecords.length > 0 && (
        <div className="bg-admin-surface border border-error rounded-lg p-5 mb-6">
          ...
          {flaggedRecords.map((r) => (
            <div key={r.id} className="flex gap-3 py-2 border-b border-admin-border last:border-b-0">
              <span className="text-meta text-admin-text flex-1">{r.title}</span>
              <Link to={`/admin/records/${r.id}`} className="text-caption text-admin-accent hover:underline">검토하기</Link>
            </div>
          ))}
        </div>
      )}
      <div className="bg-admin-surface border border-admin-border rounded-lg p-5">
        ...
        {recentRecords.map(({ record, author }) => (
          <div key={record.id} className="flex gap-3 py-2 border-b border-admin-border last:border-b-0">
            <span className="text-meta text-admin-text flex-1">{record.title}</span>
            <span className="text-caption text-admin-text-secondary">{author?.displayName}</span>
            <Link to={`/admin/records/${record.id}`} className="text-caption text-admin-accent hover:underline">보기</Link>
          </div>
        ))}
      </div>
    </div>
